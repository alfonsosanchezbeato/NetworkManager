...
verify (NMSetting *setting, NMConnection *connection, GError **error)
{
	...
	if (priv->dhcp_duid) {
		if (!_nm_utils_dhcp_duid_valid (priv->dhcp_duid)) {
			g_set_error_literal (error,
			                     NM_CONNECTION_ERROR,
			                     NM_CONNECTION_ERROR_INVALID_PROPERTY,
			                     _("invalid DUID"));
...
nm_setting_ip6_config_class_init (NMSettingIP6ConfigClass *ip6_class)
{
	 ...
	 * DUID-UUID will be generated based on the machine-id.
	 *
	 * The special values "llt" and "ll" will generate a DUID of type LLT or LL (see
	 * RFC 3315) based on the current MAC address of the device. In order to try providing
	 ...
	 *   Apart from the special values "lease", "lease-LLT", "lease-LL", "lease-UUID",
	 *   "LLT", "LL" and "UUID" a binary value in hex format is expected. An hex value
	 *   where each octet is separated by a colon is also accepted.
