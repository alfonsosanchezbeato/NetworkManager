                  ...
                  gsize *out_key_len)
{
	...
	if (g_file_get_contents (NMSTATEDIR "/secret_key", (char **) &secret_key, &key_len, &error)) {
		...
		if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND))
			nm_log_warn (LOGD_CORE, "secret-key: failure reading secret key in \"%s\": %s (generate new key)", NMSTATEDIR "/secret_key", error->message);
		g_clear_error (&error);
	}
	...
	if (!nm_utils_random_bytes (secret_key, key_len)) {
		nm_log_warn (LOGD_CORE, "secret-key: failure to generate good random data for secret-key (use non-persistant key)");
		success = FALSE;
		goto out;
	}

	if (!nm_utils_file_set_contents (NMSTATEDIR "/secret_key", (char *) secret_key, key_len, 0077, &error)) {
		nm_log_warn (LOGD_CORE, "secret-key: failure to persist secret key in \"%s\" (%s) (use non-persistant key)", NMSTATEDIR "/secret_key", error->message);
		success = FALSE;
		goto out;
	}
