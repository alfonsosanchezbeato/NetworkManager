...
static gboolean
_meta_abstract_info_is_deep_nested (const NMMetaAbstractInfo *info)
{
	nm_assert (info);

	return    info->meta_type == &nmc_meta_type_generic_info
	       && ((const NmcMetaGenericInfo *) info)->is_deep_nested;
}

static gboolean
_print_data_col_is_deep_nested (const PrintDataCol *col)
{
	nm_assert (col);

	/** a @col instance is marked as is-deep-nexsted, if the abstract-info
	 * that the col instance referens to is makred as such.
	 * ... or *any* of it's parents. */
	do {
		if (_meta_abstract_info_is_deep_nested (col->selection_item->info))
			return TRUE;
		col = col->parent_col;
	} while (col);
	return FALSE;
...
_print_data_header_get_title (const PrintDataHeaderCell *header_cell, gssize row_idx, char **out_to_free)
{
	...
	if (   row_idx >= 0
	    && _print_data_col_is_deep_nested (header_cell->col)) {
           ...
           const PrintDataCell *cells)
{
	...
	for (i_row = 0; i_row < row_len; i_row++) {
		...
		gboolean any_is_deep_nested = FALSE;
		...
		for (i_col = 0; i_col < col_len; i_col++) {
			...
			for (i_lines = 0; i_lines < lines_len; i_lines++) {
				...
				if (nmc_config->multiline_output) {
					...
					any_is_deep_nested =    any_is_deep_nested
					                     || _print_data_col_is_deep_nested (cell->header_cell->col);
