...
_metagen_con_active_vpn_get_fcn (NMC_META_GENERIC_INFO_GET_FCN_ARGS)
{
	...
	case NMC_GENERIC_INFO_TYPE_CON_VPN_VPN_STATE:
		...
		return (*out_to_free = nmc_meta_generic_get_enum_with_detail (vpn_state,
	...
	case NMC_GENERIC_INFO_TYPE_CON_VPN_CFG:
		...
		if (!s_vpn)
			return NULL;
		{
			gs_free char **arr = NULL;
			guint n;

			arr = (char **) nm_setting_vpn_get_data_keys (s_vpn, &n);
			if (!n)
				return NULL;
			...
			for (i = 0; i < n; i++) {
				...
				arr[i] = g_strdup_printf ("%s = %s", k, v);
			}

			*out_flags |= NM_META_ACCESSOR_GET_OUT_FLAGS_STRV;
			return (*out_to_free = g_steal_pointer (&arr));
		}
	default:
		break;
	}
