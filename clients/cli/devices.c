...
_metagen_device_detail_general_get_fcn (NMC_META_GENERIC_INFO_GET_FCN_ARGS)
{
	...
	case NMC_GENERIC_INFO_TYPE_DEVICE_DETAIL_GENERAL_STATE:
		...
		return (*out_to_free = nmc_meta_generic_get_enum_with_detail (state,
	...
	case NMC_GENERIC_INFO_TYPE_DEVICE_DETAIL_GENERAL_REASON:
		...
		return (*out_to_free = nmc_meta_generic_get_enum_with_detail (state_reason,
...
_metagen_device_detail_wired_properties_get_fcn (NMC_META_GENERIC_INFO_GET_FCN_ARGS)
{
	...
	case NMC_GENERIC_INFO_TYPE_DEVICE_DETAIL_WIRED_PROPERTIES_CARRIER:
		return nmc_meta_generic_get_bool (nm_device_ethernet_get_carrier (NM_DEVICE_ETHERNET (d)),
		                                  get_type);
...
_aps_to_row_data (gpointer *aps, guint len)
{
	gpointer r;
		...
		if (nmc_fields_dev_show_sections[section_idx]->nested == metagen_device_detail_ap) {
			if ((NM_IS_DEVICE_WIFI (device))) {
				...
				if (aps->len > 0) {
					...
					aps_row = _aps_to_row_data (aps->pdata, aps->len);
					nmc_print (&nmc->nmc_config,
					           (gpointer *) aps_row,
					           (gpointer) &table_data,
					           NULL,
...
do_device_wifi_list (NmCli *nmc, int argc, char **argv)
{
	...
	for (idx_d = 0; devices_all[idx_d]; idx_d++) {
		...
		g_ptr_array_add (aps_selected, NULL);
